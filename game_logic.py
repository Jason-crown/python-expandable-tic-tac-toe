...
def line_check(type, player, board, x, y, span):
    ...
    for set_1 in range(0, y if type else x):
        for set_2 in range(0, x if type else y):
            ...
            if ("".join(line_list) in "".join(anti_diagonal_list) or "".join(line_list) in "".join(diagonal_list) or "".join(line_list) in "".join(board[set_2 if type else set_1]) or "".join(line_list) in "".join(sub_board[column])) == True:
                win_game(player)
